...
def sigmoid(x):
    ...
    ### YOUR CODE HERE: sigmoid function
    s = 1/(1+np.exp(-x))
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    indices = [outsideWordIdx] + negSampleWordIndices     # the first indx is noise index
    #print('indices',indices)
    ### YOUR CODE HERE
    un, idx, n_reps = np.unique(indices, return_index=True, return_counts=True)
    U_concat = outsideVectors[un]       #k+1 * n
    n_reps[idx== 0]*=-1
    U_concat[idx !=0]*=-1
    S = sigmoid(U_concat@centerWordVec.T)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    loss = 0.0
    gradCenterVecs = np.zeros(centerWordVectors.shape)          # shape: 5,3
    gradOutsideVectors = np.zeros(outsideVectors.shape)         # shape: 5,3
    ...
    for outsideWord in outsideWords:
        ...
        temp_loss, temp_gradCenterVec, temp_gradOutsideVecs = word2vecLossAndGradient(centerWordVec,
                                outsideWordIdx,
                                outsideVectors,
                                dataset)
        loss += temp_loss
        gradCenterVecs[currentCenterWordIdx,:] += temp_gradCenterVec
        #print(gradCenterVecs)
