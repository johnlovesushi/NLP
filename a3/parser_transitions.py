...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        self.stack = ["ROOT"]           # list with the top of the stack as the last element
        self.buffer = sentence.copy()          # list
    ...
    def parse_step(self, transition):
        ...
        ###         2. Left Arc
        ###         3. Right Arc
        if transition == "S":           # represent shift
            self.stack.append(self.buffer.pop(0))
        elif transition == "LA":        # represent left arc
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        for p, t in zip(minibatch_parses, transitions): p.parse_step(t)
        # Remove the finished partial parses
        unfinished_parses[:batch_size] = [
            p for p in minibatch_parses if not (len(p.buffer) == 0 and len(p.stack) == 1)]
