...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X = self.model_embeddings.source(source_padded)                  # (src_len, b) -> src_len,b,e ->  (src_len, b, e) (22,5,3)

        '''Compute `enc_hiddens`, `last_hidden`, `last_cell` by applying the encoder to `X`'''
        X_pack = pack_padded_sequence(X,source_lengths)  #PackedSequence
        enc_hiddens,(last_hidden,last_cell) = self.encoder(X_pack)       # last_hidden: (2,b,h) last_cell: (2,b,h)

        '''Compute `dec_init_state` = (init_decoder_hidden, init_decoder_cell)'''
        enc_hiddens,_ = pad_packed_sequence(enc_hiddens)                 # (b,src_len, h)
        enc_hiddens = enc_hiddens.permute(1,0,2)
        last_hidden_cat = torch.cat([*last_hidden], dim = 1)     # hast_hidden_cat: (b,h*2)
        last_cell_cat = torch.cat([*last_cell], dim = 1)         # last_cell_cat:  (b,h*2)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        enc_hiddens_proj = self.att_projection(enc_hiddens)               #(tgt_len, b, 2*e)  -> (tgt_len, b, e)
        ...
        for Y_t in torch.split(Y,1,dim=0):
            ...
            (next_hidden_state,next_cell),o_t,_=self.step(Ybar_t,dec_state,enc_hiddens,enc_hiddens_proj,enc_masks)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_state = self.decoder(Ybar_t,dec_state)   # (b,h)*(b,src_len,h) -> (b,src_len)
        (dec_hidden, dec_cell) = dec_state
        '''Compute the attention scores e_t, a Tensor shape (b, src_len)'''
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden.unsqueeze(-1)).squeeze(-1)  # convert (b,h) -> (b,h,1), and then (b,src_len,h) * (b,h,1) = (b,src,1) -> (b,src_len)
        ### END YOUR CODE
        ...
        '''2. Calculate output vector, a_t'''
        a_t = torch.bmm(alpha_t.unsqueeze(dim=1),enc_hiddens).squeeze(dim = 1)                          # enc_hiddens is shape (b, 1, src_len) * (b, src_len, 2h),  -> (b,1,2h)
        '''3. Concatenate dec_hidden with a_t to compute tensor U_t'''
